...
-- The trigger tries to INSERT into livraisons while updating it
-- Solution: Use PRAGMA AUTONOMOUS_TRANSACTION for the INSERT
--------------------------------------------------
...
BEGIN
  ...
  -- Use autonomous transaction to avoid mutating table error
  DECLARE
  ...
  BEGIN
    ...
    INTO v_exists
    FROM livraisons
    WHERE id_entrepot_source = :NEW.id_entrepot_source
      AND id_entrepot_destination = :NEW.id_entrepot_destination
      AND statut = 'CREEE';
...
PROMPT Trigger trg_livraison_arrivee fixed to avoid mutating table error!
PROMPT The trigger now uses PRAGMA AUTONOMOUS_TRANSACTION for creating new livraisons.
